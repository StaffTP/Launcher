...
const passPhrase = "AaronSickOwnNova";
var updated = false;
let win;
...
function update() {
  ...
  let stream = request({
          uri: 'http://192.95.58.242/1.7.10.zip',
          gzip: true
      })
      .pipe(file)
      .on('finish', () => {
          ...
          pgetJSONP("http://192.95.58.242/info.json", function(data) {
              console.log(data);
              fs.readFile('./info.json', 'utf8', (err, jsonString) => {
                  if (err) {
                      console.log("File read failed:", err)
...
function launch() {
  ...
  //CHECKS FOR UPDATES
  getJSONP("http://192.95.58.242/info.json", function(data) {
      console.log(data);
      fs.readFile('./info.json', 'utf8', (err, jsonString) => {
          if (err) {
              console.log("File read failed:", err)
